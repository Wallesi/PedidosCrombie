export default function () {
    return (
        <div className="container mx-auto max-w-screen-lg pl-10 pr-10 m-28">
            <div className="space-y-4 mt-5 mb-5">

                <div className="flex items-center justify-between border border-black rounded-xl p-4">
                    <div>
                        <h3>x3</h3>
                    </div>


                    <div className="flex items-center justify-center gap-3">
                        <img src="/UserLanding/burguer.svg" className="w-16" alt="" />
                        <div>
                            <h1>Hamburguesa completa</h1>
                            <p>Con cebolla caramelizada</p>
                        </div>
                    </div>
                    <div className="flex items-center justify-center gap-3">
                        <button className="btn btn-secondary">
                            <img src="data:image/png;base64," />
                        </button>
                        <p className="text-lg">$1500</p>
                        <button className="btn btn-accent">
                            <img src="data:image/png;base64," />
                        </button>
                    </div>
                </div>

                <div className="flex items-center justify-between border border-black rounded-xl p-4">
                    <div>
                        <h3>x3</h3>
                    </div>


                    <div className="flex items-center justify-center gap-3">
                        <img src="/UserLanding/burguer.svg" className="w-16" alt="" />
                        <div>
                            <h1>Hamburguesa completa</h1>
                            <p>Con cebolla caramelizada</p>
                        </div>
                    </div>
                    <div className="flex items-center justify-center gap-3">
                        <button className="btn btn-secondary">
                            <img src="data:image/png;base64," />
                        </button>
                        <p className="text-lg">$1500</p>
                        <button className="btn btn-accent">
                            <img src="data:image/png;base64," />
                        </button>
                    </div>
                </div>

                <div className="flex items-center justify-between border border-black rounded-xl p-4">
                    <div>
                        <h3>x3</h3>
                    </div>


                    <div className="flex items-center justify-center gap-3">
                        <img src="/UserLanding/burguer.svg" className="w-16" alt="" />
                        <div>
                            <h1>Hamburguesa completa</h1>
                            <p>Con cebolla caramelizada</p>
                        </div>
                    </div>
                    <div className="flex items-center justify-center gap-3">
                        <button className="btn btn-secondary">
                            <img src="data:image/png;base64," />
                        </button>
                        <p className="text-lg">$1500</p>
                        <button className="btn btn-accent">
                            <img src="data:image/png;base64," />
                        </button>
                    </div>
                </div>

                <div className="flex items-center justify-between border border-black rounded-xl p-4">
                    <div>
                        <h3>x3</h3>
                    </div>


                    <div className="flex items-center justify-center gap-3">
                        <img src="/UserLanding/burguer.svg" className="w-16" alt="" />
                        <div>
                            <h1>Hamburguesa completa</h1>
                            <p>Con cebolla caramelizada</p>
                        </div>
                    </div>
                    <div className="flex items-center justify-center gap-3">
                        <button className="btn btn-secondary">
                            <img src="data:image/png;base64," />
                        </button>
                        <p className="text-lg">$1500</p>
                        <button className="btn btn-accent">
                            <img src="data:image/png;base64," />
                        </button>
                    </div>
                </div>
                
                <div className="flex items-center justify-evenly">
                <button className="btn bg-red-600">Cancelar pedido</button>
                <button className="btn bg-green-400">Confirmar pedido</button>
                </div>
            </div>


        </div>
    );
}
